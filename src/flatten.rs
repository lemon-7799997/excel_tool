...
impl FlattenPart {
    ...
    pub fn new(name: String, index: Option<i32>) -> Self {
        Self { name, index, nullable: false }
    }
    ...
    pub fn new_name(name: String) -> Self {
        Self { name, index: None, nullable: false }
    }
}

pub fn get_display_string(parts: &[FlattenPart]) -> String {
    let mut result = String::with_capacity(20);
    let mut prev_part: Option<&FlattenPart> = None;
    for curr in parts {
        if let Some(prev) = prev_part {
            result.push_str(if (!prev.is_index() && !curr.is_index()) || (prev.is_index() && !curr.is_index()) {
                "."
            } else {
                ""
            });
        }
        result.push_str(&if let Some(idx) = curr.index { format!("[{}]", idx) } else { curr.name.to_string() });
        prev_part = Some(&curr);
    }
    result
}
...
impl FlattenKey {
    ...
    pub fn parse<F>(raw: String, case_converter: F) -> Self
    where
        F: Fn(&str) -> String,
    {
        if raw.len() <= 0 {
    ...
    pub fn parse_with_nullable<F>(raw: String, case_converter: F) -> Self
    where
        F: Fn(&str) -> String,
    {
        if raw.len() <= 0 {
        ...
        while input.len() > 0 {
            if input.starts_with('[') {
                ...
                let segment_end = input.find('.').unwrap_or(input.len());
                // Also consider '?' as a segment boundary for nullable marker
                let question_end = input.find('?').unwrap_or(input.len());
                let name_end = match input.find('[') {
                    Some(index) if index < segment_end && index < question_end => index,
                    _ => {
                        if question_end < segment_end {
                            question_end
                        } else {
                            segment_end
                        }
            ...
            // Check for nullable marker '?' before '.'
            if input.len() > 0 && input.starts_with('?') {
...
pub fn set_json_value(json_object: &mut Value, paths: &Vec<FlattenPart>, value: Value) {
...
pub fn set_json_value_with_nullable(json_object: &mut Value, paths: &Vec<FlattenPart>, value: Option<Value>) {
    ...
    for i in 0..paths.len() {
        ...
        // If current part is nullable and current value is Null, short-circuit
        if curr_part.nullable && current.is_null() {
            return;
        }
